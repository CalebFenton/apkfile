...
public class XmlResourceMapChunk extends Chunk {
    ...
    private final Map<AttributeId, Integer> attributeIdToResourceIndex = new HashMap<>();
    ...
    protected void init(ByteBuffer buffer) {
        ...
        for (int index = 0; index < resources.size(); index++) {
            Integer attributeIdValue = resources.get(index);
            for (AttributeId attributeId : AttributeId.values()) {
                if (attributeId.value() == attributeIdValue) {
                    attributeIdToResourceIndex.put(attributeId, index);
                    break;
                }
            }
        }
    ...
    public Integer getResourceIndex(AttributeId attributeId) {
        return attributeIdToResourceIndex.get(attributeId);
    }
