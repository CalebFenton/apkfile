...
public class ArscBlamer {
    ...
    public void blame() {
        ...
        for (Entry<ResourceEntry, Collection<TypeChunk.Entry>> entry : entries.asMap().entrySet()) {
            ...
            PackageChunk packageChunk = Preconditions.checkNotNull(resourceTable.getPackage(resourceEntry.packageName()));
            ...
            for (TypeChunk.Entry chunkEntry : entry.getValue()) {
                ...
                blameKeyOrType(typeToBlame, packageChunk, chunkEntry.parent().getId() - 1, resourceEntry, typeCount);
    ...
    private void blameKeyOrType(Map<PackageChunk, List<ResourceEntry>[]> keyOrType, PackageChunk packageChunk, int keyIndex, ResourceEntry entry,
                                int entryCount) {
