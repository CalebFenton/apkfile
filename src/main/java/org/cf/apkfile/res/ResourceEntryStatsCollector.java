...
public class ResourceEntryStatsCollector {
    ...
    private void computeTypePoolSizes() throws IOException {
        for (Entry<PackageChunk, List<ResourceEntry>[]> entry : blamer.getTypeToBlamedResources().entrySet()) {
            computePoolSizes(entry.getKey().getTypeStringPool(), entry.getValue());
        }
    }

    private void computeKeyPoolSizes() throws IOException {
        for (Entry<PackageChunk, List<ResourceEntry>[]> entry : blamer.getKeyToBlamedResources().entrySet()) {
    ...
    private void computeTypeChunkSizes() {
        for (Entry<TypeChunk.Entry, Collection<ResourceEntry>> entry : blamer.getTypeEntryToBlamedResources().asMap().entrySet()) {
    ...
    private void computePackageChunkSizes() {
        for (Entry<PackageChunk, Collection<ResourceEntry>> entry : blamer.getPackageToBlamedResources().asMap().entrySet()) {
            int overhead = entry.getKey().getHeaderSize() + PACKAGE_CHUNK_OVERHEAD;
            addSizes(entry.getValue(), overhead, 0, 1);
        }
    }

    private void computePoolSizes(StringPoolChunk stringPool, List<ResourceEntry>[] usages) throws IOException {
    ...
    private void computeTypeSpecSizes(PackageChunk packageChunk, List<ResourceEntry>[] usages) {
        for (int i = 0; i < usages.length; ++i) {
